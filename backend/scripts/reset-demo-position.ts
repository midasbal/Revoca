/**
 * Resets the frontend demo's borrower (TESTNET_BORROWER_HIGH_PRIVATE_KEY,
 * tier 50 / subTier 80) to a fresh, real "valid" position on the current
 * deployed pool: real Cleanverse unfreeze, a fresh real attestation, and a
 * new real collateral post + borrow. This borrower's prior position on
 * this exact pool was already run through the full lifecycle to RESOLVED
 * earlier this session (see docs/OPEN_QUESTIONS.md), so there is nothing
 * live to show the frontend's "strike" demo against without a fresh open
 * position. Real testnet/sandbox only, no mock data.
 *
 * Run with: npx tsx scripts/reset-demo-position.ts
 */
...
async function main() {
  ...
  console.log("\n=== 1. Real update_status, unfreeze ===");
  const unfreezeResult = await client.updateStatus({
    cvRecordId: "1320",
    status: "1",
    wallet: { chain: "monad", address: borrower.address },
  });
  console.log(`  SUCCESS: ${JSON.stringify(unfreezeResult)}`);

  const facts = await client.queryApass({ chain: "monad", address: borrower.address });
  console.log(`  query_apass confirms: ${JSON.stringify(facts)}`);
  if (facts.status !== 1) {
    throw new Error(`Expected status 1 (active) after unfreeze, got ${facts.status}`);
  }

  console.log("\n=== 2. Fresh real attestation ===");
...
main().catch((err) => {
  console.error("reset-demo-position failed:", err instanceof Error ? err.message : err);
  process.exitCode = 1;
});
